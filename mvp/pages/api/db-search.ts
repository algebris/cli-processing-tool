...
function parseData(data: SearchResult[]): ParsedResult[] {
  ...
  for (const item of data) {
    const matches = item.db_search.match(/\((.*?)\)/);
    if (matches) {
      const fields = parseFields(matches[1]);
      const [id, entityId, entityType, boatModelType, title, text, tokens, similarity] = fields;
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    const pool = new Pool();
    const { rows: result } = await pool.query('SELECT db_search($1, $2, $3)', [ JSON.stringify(embedding), 0.01, matches ]);
