...
export default function Home() {
  ...
  const [mode, setMode] = useState<"search" | "chat" | "db-search" | "db-chat">("chat");
  ...
  const handleSearch = async () => {
    ...
    const results: DataChunk[] = await searchResponse.json();
  ...
  const handleAnswer = async () => {
    ...
    const prompt = endent`
    Use the following documents to provide an answer to the query: "${query}"
    ${results?.map((d: any) => `${d.title} ${d.text}`).join("\n\n")}
    `;
  ...
  const handleDatabaseSearch = async () => {
    ...
    const searchResponse = await fetch("/api/db-search", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const handleDatabaseChatSearch = async () => {
    if (!apiKey) {
      alert("Please enter an API key.");
      return;
    }
  ...
  useEffect(() => {
    ...
    if (PG_MODE) {
      setMode(PG_MODE as "search" | "chat" | "db-search" | "db-chat");
    }
  ...
  return (
    <>
      <Head>
        <title>itBoat AI Advisor</title>
        <meta name="description" content={`AI-powered search and chat`} />
          ...
          <div className="mx-auto flex h-full w-full max-w-[750px] flex-col items-center px-3 pt-4 sm:pt-8">
              ...
              <div className="w-[340px] sm:w-[400px]">
                <div>
                  ...
                  <select
                    ...
                    value={mode}
                    onChange={(e) =>
                      setMode(e.target.value as "search" | "chat" | "db-search" | "db-chat")
                    }
                  >
                    <option value="search">PDF Search</option>
                    <option value="db-search">Models Search</option>
                    <option value="chat">PDF Chat</option>
                    <option value="db-chat">Models Chat</option>
                  </select>
                </div>

                <div className="mt-2">
                  <div>Documents Count</div>
                  <input
                    type="number"
                    min={1}
                    max={10}
            ...
            {apiKey?.length === 51 ? (
              <div className="relative w-full mt-4">
                ...
                <input
                  ...
                  type="text"
                  placeholder="What is yacht broker?"
                  value={query}
            ...
            {loading ? (
              <div className="mt-6 w-full">
                ...
                <div className="font-bold text-2xl mt-6">Pdfs</div>
                <div className="animate-pulse mt-2">
                  <div className="h-4 bg-gray-300 rounded"></div>
                  <div className="h-4 bg-gray-300 rounded mt-2"></div>
                  <div className="h-4 bg-gray-300 rounded mt-2"></div>
              ...
              <div className="mt-6">
                ...
                <div className="mt-6 mb-16">
                  <div className="font-bold text-2xl">Pdfs</div>
                  ...
                  {chunks.map((chunk, index) => (
                    <div key={index}>
                      <div className="mt-4 border border-zinc-600 rounded-lg p-4">
                        ...
                        <div className="mt-2">{chunk.text}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="mt-6 pb-16">
                <div className="font-bold text-2xl">Docs</div>
                {chunks.map((chunk, index) => (
                  <div key={index}>
                    <div className="mt-4 border border-zinc-600 rounded-lg p-4">
                      <div className="flex justify-between">
